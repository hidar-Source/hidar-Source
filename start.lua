...
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/hidar-Source
token="]]..database:get(Server_Tshake.."Token_Tshake")..[["
rm -fr Tshake.lua
wget "https://raw.githubusercontent.com/hidar-Source/hidar-Source/master/hidar-Source.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./hidar-Source.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S hidar-Source -X kill
screen -S hidar-Source ./hidar-Source
done
]])
RunTs:close()
end
